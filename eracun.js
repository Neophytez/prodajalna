...
streznik.post('/izpisiRacunBaza', function(zahteva, odgovor) {
  ...
  form.parse(zahteva, function (napaka1, polja, datoteke) {
    var request = polja.seznamRacunov;
    strankaIzRacuna(request, function(napaka2, stranka) {
      pesmiIzRacuna(request, function(napaka3, pesmi) {
...
streznik.get('/izpisiRacun/:oblika', function(zahteva, odgovor) {
  pesmiIzKosarice(zahteva, function(pesmi) {
    if (!pesmi) {
      ...
      vrniStranko(zahteva.session.seja, function(napaka, stranka) {
        if(napaka) {
          odgovor.sendStatus(500);
        } else {
          odgovor.setHeader('content-type', 'text/xml');
...
streznik.post('/prijava', function(zahteva, odgovor) {
  ...
  form.parse(zahteva, function (napaka1, polja, datoteke) {
    var napaka2 = false;
    var tekst = "Stranka je bila uspešno registrirana.";
    try {
      var stmt = pb.prepare("\
    ...
    if(napaka1 || napaka2)
      tekst = "Prišlo je do napake pri registraciji nove stranke. Prosim preverite vnešene podatke in poskusite znova.";
    vrniStranke(function(napaka1, stranke) {
      vrniRacune(function(napaka2, racuni) {
        odgovor.render('prijava', {sporocilo: tekst, seznamStrank: stranke, seznamRacunov: racuni});
      })
    });
    //odgovor.end();
  });
...
streznik.post('/stranka', function(zahteva, odgovor) {
  ...
  form.parse(zahteva, function (napaka1, polja, datoteke) {
    zahteva.session.seja = polja.seznamStrank;
    odgovor.redirect('/')
  });
})
